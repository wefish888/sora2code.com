...
export default function CodeCopyButton({ code, codeId, disabled = false }: Props) {
  ...
  const handleCopy = async () => {
    ...
    setCopying(true);
    try {
      // First: copy to clipboard immediately
      await navigator.clipboard.writeText(code);
      setCopied(true);

      // Then: send API request asynchronously (don't block user experience)
  ...
  const buttonText = copying ? 'Copying...' : copied ? '✓ Copied' : 'Copy Code';
  const buttonClass = `
